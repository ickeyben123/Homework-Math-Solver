...
Class OPTIMISER : Inherits UTILITIES
    ...
    Private Function COLLECT_LIKE_TERMS(NODE As TREE_NODE)
        ...
        If NODE.VALUE = "+" Then ' Like terms can be collected.
            Console.WriteLine("BEFORE WE BEGIN" & IN_ORDER(NODE, True) & NODE.LEFT.Count & NODE.RIGHT.Count)
            For Each ELEMENT As List(Of TREE_NODE) In ITERATION_ARRAY
                For A As Integer = 0 To ELEMENT.Count() - 1
                    If (A) <= (ELEMENT.Count - 1) Then
                        ...
                        If ELEMENT_A.VALUE <> "+" Then
                            ...
                            Console.WriteLine("NODE VALUE" & NODE_LIST.Count)
                            ...
                            If Not ELEMENT_B Is Nothing Then ' As it is an 'each' loop, I will need to modify it outside of it. 
                                Console.WriteLine("finished boi BEFORE")
                                Console.WriteLine(IN_ORDER(TREE_TO_MODIFY, True))
                                Console.WriteLine(IN_ORDER(ELEMENT_A, True) & " the fukc" & IN_ORDER(ELEMENT_B, True) & "the root" & NODE.VALUE)
                                ...
                                End If
                                Console.WriteLine("finished boi vFUFUFUUFUF")
                                Console.WriteLine(IN_ORDER(TREE_TO_MODIFY, True))
                    ...
                    End If
                Next
                Console.WriteLine("end of NODE LIST" & NODE_LIST.Count)
            Next
        End If
        If Not NODE.LEFT Is Nothing Then
...
Module MODULE1
    ...
    Sub MAIN()
        Dim SIMPLIFIED As New SIMPLE_SIMPLIFY("x*x^(9+(9+x)*(x^2+x))")
